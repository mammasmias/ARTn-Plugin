...
SUBROUTINE plugin_ext_forces(force)
  ...
  ELSE IF ( lperp .and. .not. llanczos ) THEN
     ...
     IF ( leigen ) THEN
        ...
        CALL perpforce(force,eigenvec,fpara,nat)
        CALL perpmove(nat,istepperp,eigenvec)
        istepperp = istepperp + 1 
     ELSE
        ! write (*,*) "ARTn: force full:",force(:,:)
        CALL perpforce(force,push,fpara,nat)
        CALL perpmove(nat,istepperp,push)
        istepperp = istepperp + 1 
        WRITE (*,*) "ARTn: Dot product between push and perp force", ddot(3*nat, force(:,:), 1, push(:,:),1)
     ENDIF
     !
     !
     ...
     IF ( nlanc > nlanciter ) THEN
        ...
        IF ( lowest_eigval < 0.0_DP ) THEN
          !  WRITE (*,*) "Coordinates of lanczos end:", tau(:,:)
           WRITE (*,*) "ARTn: found negative eigenvalue, using eigvec:",eigenvec(:,:)
...
SUBROUTINE lanczos( nat, force, v_in, nlanciter, nlanc, lowest_eigval, lowest_eigvec, pushdir)
  ...
  REAL(DP) :: dlanc
  REAL(DP), PARAMETER :: eigvec_thr = 1.0D-3, eigval_thr = 1.0D-3
  ...
  ELSEIF (nlanc > 1 .and. nlanc <= nlanciter ) THEN
     ...
     !
     eigval_diff = (lowest_eigval - lowest_eigval_old)/lowest_eigval
