  ...
  IF ( lperp ) THEN 
     ...
     IF ( leigen ) THEN 
        ...
        IF( C1 .OR. C2 .OR. C3 )THEN
           ...
           leigen = .false. 
           ilanc = 0
           IF ( C1.AND.verbose>2 ) WRITE(iunartout,*)"   |> Stop perp relax because fperp < fperp_thr ",&
               unconvert_force( maxfperp ), unconvert_force(fperp_thr), TRIM(converge_property) 
           IF ( C2.AND.verbose>2 ) WRITE(iunartout,*)"   |> Stop perp relax because iperp > nperp_thr ",&
               iperp, nperp 
           IF ( C3.AND.verbose>2 ) WRITE(iunartout,*)"   |> Stop perp relax because fperp < fpara ",&
